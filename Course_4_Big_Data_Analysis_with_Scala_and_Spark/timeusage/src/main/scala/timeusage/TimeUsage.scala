...
object TimeUsage {
  ...
  def timeUsageByLifePeriod(): Unit = {
    ...
    // val finalDf2 = timeUsageGroupedSql(summaryDf).collect()
    // val finalDf3 = timeUsageGroupedTyped(timeUsageSummaryTyped(summaryDf)).collect()
  ...
  def row(line: List[String]): Row = {
    val castedLine = line.head :: line.tail.map(x => x.toDouble)
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    var primaryCol: List[Column] = Nil
    var workingCol: List[Column] = Nil
    var leisureCol: List[Column] = Nil

    val primaryKeys = List("t01", "t03", "t11", "t1801", "t1803")
    val workingKeys = List("t05", "t1805")
    val leisureKeys = List("t02", "t04", "t06", "t07", "t08", "t09", "t10", "t12", "t13", "t14", "t15", "t16", "t18")

    for (colName <- columnNames) {
      if (primaryKeys.exists(colName.startsWith))
        primaryCol  = col(colName) :: primaryCol
      else if (workingKeys.exists(colName.startsWith))
        workingCol = col(colName) :: workingCol
      else if (leisureKeys.exists(colName.startsWith))
        leisureCol = col(colName) :: leisureCol
    }

    (primaryCol, workingCol, leisureCol)
  }
  ...
  def timeUsageSummary(
    ...
    val workingStatusProjection: Column = when(df("telfs") >= 0 && df("telfs") < 3, "working")
