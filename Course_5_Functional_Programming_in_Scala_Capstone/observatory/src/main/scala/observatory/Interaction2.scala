...
object Interaction2 {
  ...
  def availableLayers: Seq[Layer] = {
    val temperatureColors: Seq[(Double, Color)] =
      Seq((60, Color(255, 255, 255)),
    ...
    val temperatureLayer = Layer(LayerName.Temperatures, temperatureColors, 1975 to 2015)

    val deviationColors: Seq[(Double, Color)] =
      Seq((7, Color(0, 0, 0)),
        (4, Color(255, 0, 0)),
        (2, Color(255, 255, 0)),
        (0, Color(255, 255, 255)),
    ...
    val deviationLayer = Layer(LayerName.Deviations, deviationColors, 1990 to 2015)
  ...
  def yearSelection(selectedLayer: Signal[Layer], sliderValue: Signal[Int]): Signal[Int] = {
    ...
    Signal(sliderValue() max yearRange().min min yearRange().max)
  }
  ...
  def layerUrlPattern(selectedLayer: Signal[Layer], selectedYear: Signal[Int]): Signal[String] = {
    val yearSelected = yearSelection(selectedLayer, selectedYear)
    Signal(s"./target/${selectedLayer().layerName.id}/${yearSelected()}/{z}/{x}-{y}.png")
